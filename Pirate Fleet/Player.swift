...
//        }
    ...
    func addPlayerShipsMinesMonsters(_ numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        func mineRandomizer(location: GridLocation) -> Mine {
            var mine : Mine
            let choice = arc4random() % 2
            if choice == 0 {
                mine = Mine(location: location, penaltyText: "Slow down!")
            } else {
                mine = Mine(location: location, penaltyText: "Ka-boom!", guaranteesHit: true)
            }
            return mine
        }
